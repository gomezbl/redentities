...
class SqlEntities {
    constructor(databaseConfig, databaseSchema, connector, formatters, databaseRemover, databaseCreator ) {
    ...
    async RenameSchemaEntities( sufix ) {
        ...
        for( let entity of this.DatabaseSchema.entities ) {            
            ...
            sql.push( this.Formatters.FormatRenameTable( entity.name, `${newTableName};`) );
    ...
    async ExistsSchema() {
        let sql = this.Formatters.FormatSelectDatabaseAndTable(this.DatabaseConfig.database, this.DatabaseSchema.entities[0].name );
    ...
    async CreateEntity( entitySchema, removeIfExists ) {        
        ...
        if ( existsEntity && removeIfExists ) {
            let dtb = new DropTableBuilder( entitySchema.name, this.Formatters );
            sql.push( dtb.Query() );
        }

        let ctb = new CreateTableBuilder(entitySchema.name, this.Formatters);
    ...
    async CreateSchema(removesIfExists) {
        ...
        let existsDatabase = await this.ExistsDatabase(this.DatabaseConfig.database);
    ...
    async RemoveSchema(schema) {
        ...
        for(let entity of this.DatabaseSchema.entities ) {
            let dropTableBuilder = new DropTableBuilder( entity.name, this.Formatters );
    ...
    async ExistsDatabase( databaseName ) {
        let sql = this.Formatters.FormatShowDatabase( databaseName );
        let result = await this.RunQuery(sql);
    ...
    async ExistsTable( tableName ) {        
        let sql = this.Formatters.FormatShowTable(tableName);
        let result = await this.RunQuery(sql);
    ...
    async RemoveDatabase( databaseName ) {
        return this.DatabaseRemover.RemoveDatabase( databaseName, this.Formatters, this.Connector, this.DatabaseConfig );
    }

    async CreateDatabase( databaseName ) {
        return this.DatabaseCreator.CreateDatabase( databaseName, this.Formatters, this.Connector, this.DatabaseConfig );
    }

    Insert( entityName ) {
        return new InsertQueryBuilder( entityName, this, this.Formatters );
    }

    Update( entityName ) {
        return new UpdateQueryBuilder( entityName, this, this.Formatters );
    }

    Select( entityName ) {
        return new SelectQueryBuilder( entityName, this, "*", this.Formatters );
    }

    Delete( entityName ) {
        return new DeleteQueryBuilder( entityName, this, this.Formatters );
    }
    ...
    async RunQuery( sql ) {
        let result = await this.Connector.RunQuery( this.DatabaseConfig, sql );
