...
class SqlEntities {
    ...
    async CreateEntity( entitySchema, removeIfExists ) {        
        ...
        if ( existsEntity && removeIfExists ) {
            sql.push( new DropTableBuilder( entitySchema.name, formatters ) );
        }
    ...
    async ExistsTable( tableName ) {        
        const result = await this.RunQuery( this.Adaptors.Formatters.FormatShowTable(tableName) );
...
function populateEntities(sqlEntities) {
    ...
    for( const entity of sqlEntities.DatabaseSchema.entities ) {
        sqlEntities[entity.name] = {
            ...
            S : function(fields) { return new SelectQueryBuilder( entity.name, sqlEntities, fields ? fields : "*", formatters ) },
