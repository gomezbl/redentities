...
module.exports = {
    ...
    FormatWhere: (expression, values) => {
        return MySql.format(expression, values);
    },
    ...
    FormatCreateTable: (tableName, fields) => {
        ...
        for( var i = 0; i < fields.length; i++ ) {
            ...
            c = Replace( c, "fieldtype", TypeToMysqlStype(fields[i].type) );            
            f += c;
        }
    ...
    FormatIndex: (tableName, fields, indexCount) => {              
        return `CREATE INDEX ${INDEXPREFIX}_${indexCount} ON ${tableName}(${fields.join(",")})`;
    },

    FormatUniqueIndex: (tableName, fields, indexCount) => {
        return `CREATE UNIQUE INDEX ${UNIQUEINDEXPREFIX}_${indexCount} ON ${tableName}(${fields.join(",")})`;
    },
...
function TypeToMysqlStype( type, mysqltype ) {
    switch( type ) {
